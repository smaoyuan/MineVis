...
/**
 * Print Line Helper
 * @param string $string text to print on a line
 */
function pl($string) {
    echo $string . "\n";
}
...
//$parser->parse('/Users/thepunksnowman/Dropbox/Research/VastSets/Manpad.jig');
//$parser->parse('/Users/thepunksnowman/Dropbox/Research/VastSets/crescent.jig');
//$parser->parse('/Users/thepunksnowman/Dropbox/Research/VastSets/AtlanticStorm.jig');
//$parser->parse('/Users/thepunksnowman/Dropbox/Research/VastSets/Stegosaurus.jig');
//$parser->parse('/Users/thepunksnowman/Dropbox/Research/VastSets/vast2010.jig');
$parser->parse('/Users/thepunksnowman/Dropbox/Research/VastSets/vast_2011_vastopolis.jig');

$t1 = microtime(true);
$diff = ($t1 - $t0);
pl("\nParsing Complete (runtime: $diff)");
pl("\nStats:\n");
pl("Peak Memory Usage: " . memory_get_peak_usage() / 1024 / 1024 . "MB");
pl("Total Documents Parsed: " . $parser->getDocumentCount());
$types = $parser->getEntityTypes();
pl("Unique Entity Types:");
foreach ($types as $t) {
    pl("\t*" . $t);
}
...
$output = fopen('./jigsaw.sql', 'w');
$parser->writeSQL($output);
fclose($output);

$t1 = microtime(true);
$diff = ($t1 - $t0);
pl("\nOutput Complete (runtime: $diff)");
pl("Peak Memory Usage: " . memory_get_peak_usage() / 1024 / 1024 . "MB");
/*
 * End Test stuff
 */
